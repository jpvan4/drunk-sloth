...
async fn run_lll_reduction(
    ...
    let reduced_lattice = if args.gpu && features::gpu_available() {
...
async fn run_bkz_reduction(
    ...
    let start_time = Instant::now();
    let reduced_lattice = if args.gpu && features::gpu_available() {
        log::info!("Using GPU-accelerated BKZ");
        match GPUAcceleratedOperations::new().await {
            Ok(gpu_ops) => gpu_ops.accelerated_bkz_reduction(&lattice).await?,
            Err(e) => {
                log::warn!("Failed to initialize GPU for BKZ: {}. Falling back to CPU BKZ.", e);
                let reducer = BKZReducer::with_params(params);
                reducer.reduce(&lattice)?
            }
        }
    } else {
        log::info!("Using CPU-only BKZ");
        let reducer = BKZReducer::with_params(params);
        reducer.reduce(&lattice)?
    };
    let execution_time = start_time.elapsed();
...
async fn run_cvp_solver(
    ...
    params.preprocessing_algorithm = match preprocessing_algo {
...
async fn run_benchmark(
        ...
        for _ in 0..runs {
            ...
            for &algo in &algorithms {
                ...
                let benchmark_result = BenchmarkResult {
                    ...
                    quality_metrics: test_lattice.quality_metrics()?,
                    gpu_used: false,
    ...
    if compare_gpu && args.gpu && features::gpu_available() {
        ...
        for &dim in &dimensions {
            let test_lattice = generate_random_lattice_cli(dim, dim, None)?;
            let benchmark_result = gpu_ops.benchmark_gpu_performance(test_lattice.basis()).await?;
